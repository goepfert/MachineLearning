...
const App = (() => {
  ...
  /**
   * Load and random image from imageDataset
  ...
  function loadAndDrawRandomImage() {
    ...
    // image = Utils.add_SP_Noise(image, noise_prob, 1);
    image = Utils.add_Gaussian_Noise(image, noise_prob, 1);
    currentData.data = image;

    // image = noise_fcn(image, noise_prob, 1);
   ...
   * Converts pixel range from (0,255) -> (1,0)
   */
  ...
  async function train() {
    ...
    const trainingData = imageDataset.getTrainingData(noise_fcn, noise_prob);
    await nn.train(trainingData.xNoisy, trainingData.x, model);
