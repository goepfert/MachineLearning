...
function createImageDataset(img_width, img_height) {
  ...
  function nextTestBatch(size, randomize = true, noise_fcn = undefined, noise_prob = 0.1) {
    ...
    for (let idx = 0; idx < xData.length; idx++) {
      xData_noisy.push(noise_fcn(xData[idx], noise_prob, 1));
    }
  ...
  function getTrainingData(noise_fcn, noise_prob) {
    const nDatasets = _data.length;

    return nextTestBatch(nDatasets, false, noise_fcn, noise_prob);
  }
